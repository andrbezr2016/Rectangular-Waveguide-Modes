...
class Graf:
	...
	def makeTMnm(xx,yy,time,omeg,kp,kpX,kpY,hh,self):
		...
		z4 = arange(h, self.lamb, h)
		xgrid4, zgrid4 = meshgrid(x4, z4)
		ygrid4 = (abs(cos(kpX*xgrid4))**((kp/kpX)**2))*abs(cos(omeg*time-hh*zgrid4))*sin(kpY*yy) #синус ли здесь в конце?
		#TM Ezy
		z5 = arange(h, self.lamb, h)
		y5 = arange(h, self.b, h)
		zgrid5, ygrid5 = meshgrid(z5, y5)
		xgrid5 = (abs(cos(kpY*ygrid5))**((kp/kpY)**2))*abs(cos(omeg*time-hh*zgrid5))*sin(kpX*xx) #синус ли здесь в конце?
		return xgrid, ygrid, zgrid, xgrid1, ygrid1, zgrid1, xgrid2, ygrid2, zgrid2, xgrid3, ygrid3, zgrid3, xgrid4, ygrid4, zgrid4, xgrid5, ygrid5, zgrid5
	...
	def makeTEn0(xx,yy,time,omeg,kp,kpX,kpY,hh,self):
		...
		y3 = arange(h, self.b, h)
		xgrid3, ygrid3 = meshgrid(x3, y3)
		zgrid3 = cos(kpX*xgrid3)*sin(omeg*time) ############################# sin
		#TE Exz
		x4 = arange(h, self.a, h)
		z4 = arange(h, self.lamb, h)
		xgrid4, zgrid4 = meshgrid(x4, z4)
	...
	def makeTE0m(xx,yy,time,omeg,kp,kpX,kpY,hh,self):
		...
		y3 = arange(h, self.b, h)
		xgrid3, ygrid3 = meshgrid(x3, y3)
		zgrid3 = cos(kpY*ygrid3)*sin(omeg*time) ################### sin
		#TE Exz
		x4 = arange(h, self.a, h)
		z4 = arange(h, self.lamb, h)
		xgrid4, zgrid4 = meshgrid(x4, z4)
		...
		if omeg>omegK:
			if self.Moda==0:
				...
				if self.n!=0 and self.m!=0:
					...
					if self.m%2==0 and self.m!=0:
						N2=2 #четное ли б, если да то делим б на 2 для выбора сечения волновода в нужном месте
					x, y, z, x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, x5, y5, z5 = Graf.makeTMnm(self.a/(2*self.n),self.b/(2*self.m),time,omeg,kp,kpX,kpY,hh,self)
					subplot(2,4,1)
...
if __name__ == "__main__":
	...
	def Vvod(time):
		...
		(message5.get()).replace('.','',1).isdigit() and float(message3.get())!=0 and float(message4.get())!=0 and float(message5.get())!=0):
			...
			plot1 = Graf(Moda,float(message1.get()),float(message2.get()),float(message3.get()),float(message4.get()),float(message5.get()))
